...
def imputation_with_mice_rf(data):
...
data_nan_imputed= imputation_with_mice_rf(df_nan)
...
#final_data.to_csv('final_data_imputed_with_MICE-RF.csv', index=False)
#final_data.to_excel('final_data_imputed_with_MICE-RF.xlsx', index=False)

full_data_imputed = imputation_with_mice_rf(data)
...
test_with_and_without_standardization(x_train, x_test, y_train, y_test, MLPClassifier(random_state=42, max_iter=1000), "MLP Classifier")
# Save the final data
#full_data_imputed.to_csv('full_data_imputed_with_MICE-RF_2.csv', index=False)
#full_data_imputed.to_excel('full_data_imputed_with_MICE-RF_2.xlsx', index=False)
